...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=8, zero_init_residual=False,
        ...
        # self.conv1_1 = nn.Conv2d(4, self.inplanes, kernel_size=7, stride=1, padding=1, bias=False, groups=2)
        # self.conv1_2 = nn.Conv2d(4, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        
        # self.weight = nn.Parameter(torch.randn(64, self.inplanes, 3, 3))
        # self.offset = nn.Parameter(torch.rand(4, 2 * 3 * 3, 112, 112))
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(url = model_urls[arch], map_location='cpu', check_hash=True)
