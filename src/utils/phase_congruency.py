...
def _phase_congruency(x: torch.Tensor, scales: int = 4, orientations: int = 6,
                      min_length: int = 3, mult: int = 1.6, sigma_f: float = 0.55,
                      delta_theta: float = 1.2, k: float = 2.0) -> torch.Tensor:
    ...
    N, C, H, W = x.shape
    if C != 1:
        x = x[:,0:1] * 0.299 + x[:,1:2] * 0.587 + x[:,2:] * 0.114
    ...
    an = torch.sqrt(torch.sum(even_odd ** 2, dim=-1)+1e-9)
    ...
    x_energy = torch.sqrt(sum_e ** 2 + sum_o ** 2+1e-9) + EPS
    ...
    abs_eo = torch.sqrt(torch.sum(even_odd[:, :, :1, ...] ** 2, dim=-1)+1e-9).reshape(N, orientations, 1, 1, H * W)
    ...
    # Rayleigh parameter
    tau = torch.sqrt((noise_energy2 / 2)+1e-9)

    # Expected value of noise energy
    noise_energy = tau * math.sqrt(math.pi / 2)
    moise_energy_sigma = torch.sqrt(((2 - math.pi / 2) * tau ** 2)+1e-9)
