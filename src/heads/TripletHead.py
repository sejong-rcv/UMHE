    ...
    def forward(self, data):
        ...
        if isinstance(self.triplet_margin, str):
            ...
            if self.triplet_channel_aggregation == 'channel-aware':
                ...
                loss_mat_1 = torch.max(torch.sum(l1, dim=1) - torch.sum(l3, dim=1) + self.triplet_margin,torch.zeros_like(l1))
            else:
                assert False, 'Do not know this aggregation technique'
        ...
        if self.variant == 'doubleline':
            ...
            if isinstance(self.triplet_margin, str):
                ...
                if self.triplet_channel_aggregation == 'channel-aware':
                    ...
                    loss_mat_2 = torch.max(torch.sum(l2, dim=1) - torch.sum(l3, dim=1) + self.triplet_margin,torch.zeros_like(l2))
                else:
                    assert False, 'Do not know this aggregation technique'
